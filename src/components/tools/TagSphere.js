...
const updateItemPosition = (item, sc, depth) => {
    ...
    // @ts-ignore
    // itemEl.style.filter = `grayscale(${(alpha - 1) * -8}) blur(${(alpha - 1) * -5 > 1 ? Math.floor((alpha - 1) * -8) : 0
    //     }px)`;
...
const createItem = (text, index, textsLength, size, itemRef) => {
    ...
    return { ref: itemRef, el: itemEl, ...computeInitialPosition(index, textsLength, size) };
};

const defaultState = {
    tags: ['This', 'is', 'TagSphere.', 'Do', 'you', 'like', 'it?', 'Glad', 'to', 'see', 'you'],
    maxSpeed: 5,
    initialSpeed: 20,
...
const TagSphere = (props) => {
    ...
    if (!radius) {
        radius = tags.length * 15;
    }

    const depth = 2 * radius;
    const size = 1.5 * radius;
    const itemHooks = tags.map(() => React.createRef());
    ...
    const [lessSpeed, setLessSpeed] = React.useState(maxSpeed);
